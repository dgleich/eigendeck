...
function page({ title, bodyHtml, activeOut, sidebar: withSidebar = true }) {
  // LLM-disclosure note at the TOP of the reading column — shown before the
  // content, so readers know before they read. See docs discussion.
  const note = '<p class="llm-note">Content generated by an LLM with human direction.</p>';
  const body = note + bodyHtml;
  const layout = withSidebar
    ? `<div class="manual-layout">${sidebar(activeOut)}<main class="manual-content">${body}</main></div>`
    : `<div class="manual-layout learn"><main class="manual-content">${body}</main></div>`;
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
