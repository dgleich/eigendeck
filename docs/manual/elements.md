...
and per-element color, background panel, alignment, opacity, rotation, and
...
The **Note** preset doubles as the annotation: smaller, italic, accent-colored
...
A straight arrow with adjustable color, stroke width, and head size — for
pointing an annotation at the thing it describes.

## Cover elements

A rectangle that **defaults to the slide background color**, so it's an
invisible mask — drop it over content you'll reveal on a later build slide, then
remove it. You can also tint a cover any color from the inspector (Color →
