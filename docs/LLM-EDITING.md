...
- `footerFont`: optional font package id for the slide footer (the author·venue line + slide number). Unset → `"lato"` (the deck default font). Deck-level only.
- `textSizes`: optional partial map overriding the deck's named type scale. Keys: `"footnote"` (default 24), `"note"` (32), `"body"` (48), `"title"` (62), `"hype"` (48). Values in slide-pixels. Absent keys fall back to these `DEFAULT_TEXT_SIZES` defaults — which apply to every deck without its own stored size (old decks included), so `"title"` is 62 everywhere (lowered from 72 so a two-line default title fits its 180px box). New decks additionally STAMP the full scale into `config.textSizes` so their look is frozen against future default changes, and a deck opened without a stored scale is back-filled with the defaults on open. Used by every element that picks a size by name (notebook `fontSizeName`, and — as text presets are retrofitted — text element sizes).
...
|-------------|----------|---------------------|------------|-----------|---------|-----------------|
| `title`      | 62       | PT Sans             | bold       | normal    | #222    | `x:60, y: 60, 1800×180` (bottom-aligned) |
