...
- **Demos (security isolation).** Switching to a demo slide re-creates the
  ...
  pre-security **26 ms**/switch → post-security **96 ms**. Breakdown of the ~70 ms gap:
    1. **Embedded `@font-face` re-parse (~28 ms).** An opaque iframe can't fetch
       ...
       of `magnetic-powers`/`welcome`) still pays it, but the payload is now
       **WOFF2** (Brotli, ~40% of the TTF): `buildEmbeddedFontFacesCSS`
       (`fonts.ts`) prefers the `.woff2` sibling generated by
       `tools/build_font_woff2.py` (run from `npm run setup`; gitignored; falls
       back to TTF if absent). `magnetic-powers` rapidSlideNav 119→91,
       presentAdvance 74→53. Embedding only declared weights is a further lever.
