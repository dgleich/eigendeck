...
writeFileSync(join(HOME,'images','a.svg'),'AA');        // 2 bytes
writeFileSync(join(HOME,'images','b.svg'),'BB');        // 2
writeFileSync(join(HOME,'images','sub','c.svg'),'CC');  // 2
...
for(const [p,id] of [['images/a.svg','ra'],['images/b.svg','rb'],['images/sub/c.svg','rc']]){
  const enc='AA'; // store actual 2-byte content
  const r=await execA(sid,`const d=arguments[arguments.length-1];window.__TAURI_INTERNALS__.invoke('db_store_asset',{path:'${p}',data:[65,65],mimeType:'image/svg+xml',externalPath:'${p}',externalMtime:null,assetId:'${id}'}).then(()=>d('ok')).catch(e=>d('ERR'+e));`);
...
writeFileSync(join(HOME,'moved','images','a.svg'),'AAAAA');     // 5 bytes
writeFileSync(join(HOME,'moved','images','b.svg'),'BBBBBB');    // 6
writeFileSync(join(HOME,'moved','images','sub','c.svg'),'CCCCCCC'); // 7
...
await execA(sid,`const d=arguments[arguments.length-1];window.__TAURI_INTERNALS__.invoke('db_store_asset',{path:'images/a.svg',data:[65,65,65,65,65],mimeType:'image/svg+xml',externalPath:${JSON.stringify(newAbs)},externalMtime:null,assetId:'ra'}).then(()=>d('ok')).catch(e=>d('ERR'+e));`);
...
await quit(sid);
if(lb!==6) fail(`rb did not re-read moved bytes (len=${lb}, want 6)`);
if(lc!==7) fail(`rc did not re-read moved bytes (len=${lc}, want 7)`);
