...
//   2. TRUST + WATCH        → mutate clip.mp4 → reloads (trusted + approved path)
...
//   5. STOP TRUSTING        → mutate clip.mp4 → NO reload (trust+approvals dropped)
//
// Trust / approve / revoke are driven through the REAL Security window (window-handle
// switch + real button clicks), NOT action seams. Byte-level (no codecs), so it runs
// headlessly. Needs an empty deck under HOME (fs:allow-watch scope).
import { writeFileSync } from 'fs';
import { dirname, join } from 'path';
import { openApp, waitSeam, quit, handles, exec, execA, sleep,
         trustAndWatchAllViaUI, revokeViaUI } from './_ui.mjs';
...
// ── 2. TRUST + WATCH (real Security window) → reload ─────────────────────────
if (!await trustAndWatchAllViaUI(sid, mainH)) fail('trust+watch via UI did not land');
// Approving pulls the file's CURRENT on-disk bytes (5000, changed in step 1) and, because
...
if (!await waitLen(sid,'va1',5000)) fail('2a) trust+approve did not pull the current on-disk bytes');
await sleep(2500);
writeFileSync(CLIP, mp4(7000, 3));
if (!await waitLen(sid,'va1',7000)) fail('2) TRUSTED+approved file did NOT live-reload');
console.log('  2) trust+watch → clip.mp4 pulled (5000) then live-reloads on edit (→ 7000) ✓');
...
// ── 4. APPROVE the new path (real Security window, approve-folder) ───────────
if (!await trustAndWatchAllViaUI(sid, mainH)) fail('approve clip2 via UI did not land');
...
// ── 5. STOP TRUSTING → back to snapshot (no reload) ─────────────────────────
if (!await revokeViaUI(sid, mainH)) fail('revoke via UI did not land');
...
console.log('  5) stop trusting → clip.mp4 change NOT read (reverts to snapshot, stays 7000) ✓');
