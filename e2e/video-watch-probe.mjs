...
writeFileSync(CLIP, Buffer.alloc(1000, 1));   // v1
const sid = await open(); if (!sid || !await waitSeam(sid)) fail('open');
await execA(sid, "const d=arguments[arguments.length-1];window.__TAURI_INTERNALS__.invoke('db_store_asset',{path:'clip.mp4',data:Array.from(new Uint8Array(1000).fill(1)),mimeType:'video/mp4',externalPath:'clip.mp4',externalMtime:null,assetId:'va1'}).then(()=>d('ok')).catch(e=>d('ERR'+e));");
...
writeFileSync(CLIP, Buffer.alloc(5000, 2));   // v2 — mutate on disk
let after = before;
