...
soft('shows the linked external path', /images\/dot\.png/.test(apText), apText.slice(0, 120));
...
// ── 6. missing-source banner (#74) — may or may not be flagged; soft-observe ──
const missingBanner = /Source file is missing|Relocate/.test(String(await assetSectionText() || ''));
soft('missing-source banner shown for the dangling link', missingBanner, 'not flagged by scan (ok)');
...
// ── 8. PDF asset (pdfium) — soft: only if the dylib is present in the rig ──────
console.log('\n[8] pdf asset render (pdfium — soft)');
await selectAsset('apdf');
await sleep(1500);
soft('pdf canvas element rendered an <img> (pdfium available)', await exec(sid, "return !!document.querySelector('[data-element-id=\"apdf\"] img')"), 'pdfium dylib not in rig — expected soft-skip');
...
const realErrs = errs.filter((e) => !/network|Failed to fetch|load|ERR_|asset|db_get|not found|No such|render/i.test(e));
...
if (problems.length > 7) fail(`too many asset steps failed (${problems.length}) — likely a structural break`);
