# Eigendeck Development Log — March 31 – April 2, 2026 (founding)

> Migrated from the old root `CHANGELOG.md` (2026-07-07) so the founding work
> lives with the rest of the session logs. This is the origin story: scaffold →
> reveal.js removal → unified elements → text presets → MathJax. `git log` has the
> commit-level detail; this is the narrative.
