...
1. **Author a builder** (recommended) — a small Node script that emits the deck
   ...
   regenerable and diff-able. Keep HTML self-contained (inline `<style>`,
   system/`data:` fonts, inline SVG / `data:` images).
   **Always include a top-level `config: { "width": 1920, "height": 1080 }`** in
   the deck JSON so the slide stage is sized correctly.
2. **Build the deck** with the **eigendeck-cli** skill:
   `eigendeck-cli <deck>.eigendeck import json <deck>.json`.
3. **Open the deck in Eigendeck and eyeball EVERY slide** — don't trust the JSON.
   Page through in the editor (and try Present) and actually LOOK. Fix anything
   clipped, off-canvas, or where a font/image silently dropped (the sandbox blocks
   the network, so a remote font/image renders as its fallback — inline `data:`
   URIs or a system stack instead). If you intended a custom face, confirm it's
   actually the one rendering, not a fallback.
4. **Save.** The deck is a single `.eigendeck` file; keep your builder script next
   to it so the deck stays regenerable.
