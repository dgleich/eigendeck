...
fn build_app_menu(app: &tauri::AppHandle, recent_menu: Option<tauri::menu::Submenu<tauri::Wry>>) -> Result<tauri::menu::Menu<tauri::Wry>, String> {
    ...
    let insert_menu = SubmenuBuilder::new(app, "Insert")
        ...
        // Insert an HTML element from a .html snippet file (validated first).
        .item(&MenuItemBuilder::new("HTML Element from File…").id("insert-html-file").build(app).map_err(|e| e.to_string())?)
        .separator()
