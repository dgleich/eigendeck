// Aggressive, adversarial coverage of notebookOverlay.ts — the
// merge/overlay engine. The existing notebookOverlay.test.ts holds a
...
describe('parseOverlay — happy paths & input forms', () => {
  ...
  it('parses from a Uint8Array', () => {
    ...
    r.cellEdits[3] = 'x';
    const bytes = new TextEncoder().encode(serializeOverlay(r));
    expect(parseOverlay(bytes)).toEqual(r);
  });

  it('parses from an ArrayBuffer', () => {
    const r = emptyOverlay();
    r.cellEdits[3] = 'x';
    const u8 = new TextEncoder().encode(serializeOverlay(r));
    // copy into a standalone ArrayBuffer to exercise the ArrayBuffer branch
    const ab = u8.buffer.slice(u8.byteOffset, u8.byteOffset + u8.byteLength);
    expect(parseOverlay(ab)).toEqual(r);
  });

  it('empty object → emptyOverlay', () => {
    expect(parseOverlay('{}')).toEqual(emptyOverlay());
  });

  it('always normalizes version to literal 1, even if absent or wrong', () => {
    expect(parseOverlay('{"version":99}').version).toBe(1);
    expect(parseOverlay('{}').version).toBe(1);
    expect(parseOverlay('{"version":"banana"}').version).toBe(1);
  });
});

describe('parseOverlay — malformed / hostile input → empty', () => {
  it('not JSON at all', () => {
    expect(parseOverlay('not json')).toEqual(emptyOverlay());
  });

  it('truncated JSON', () => {
    expect(parseOverlay('{"cellEdits": {')).toEqual(emptyOverlay());
  });
  ...
  it('JSON primitives (number/string/bool) → empty', () => {
    expect(parseOverlay('42')).toEqual(emptyOverlay());
    expect(parseOverlay('"a string"')).toEqual(emptyOverlay());
    expect(parseOverlay('true')).toEqual(emptyOverlay());
  });
  ...
  it('JSON array at top level → empty', () => {
    ...
    expect(parseOverlay('[1,2,3]')).toEqual(emptyOverlay());
  });
});

describe('parseOverlay — wrong-typed top-level fields are coerced/dropped', () => {
  it('cellEdits as a number → {}', () => {
    expect(parseOverlay('{"cellEdits": 42}').cellEdits).toEqual({});
  });
  ...
  it('cellEdits as an array → entries that happen to be strings survive (documents current behavior)', () => {
    ...
    const r = parseOverlay('{"cellEdits": ["a", "b"]}');
    expect(r.cellEdits).toEqual({ 0: 'a', 1: 'b' });
  });

  it('cellEdits as null → {}', () => {
    expect(parseOverlay('{"cellEdits": null}').cellEdits).toEqual({});
  });

  it('cellOutputs not an array-of-arrays → non-array values dropped', () => {
    const r = parseOverlay('{"cellOutputs": {"0": "nope", "1": [], "2": 5}}');
    expect(r.cellOutputs).toEqual({ 1: [] });
  });

  it('cellOutputs as a non-object → {}', () => {
    expect(parseOverlay('{"cellOutputs": 7}').cellOutputs).toEqual({});
    expect(parseOverlay('{"cellOutputs": "x"}').cellOutputs).toEqual({});
  });

  it('cellCounts accepts number and null, drops everything else', () => {
    const r = parseOverlay('{"cellCounts": {"0": 3, "1": null, "2": "x", "3": [], "4": {}}}');
    expect(r.cellCounts).toEqual({ 0: 3, 1: null });
  });

  it('appendedCells not an array → []', () => {
    expect(parseOverlay('{"appendedCells": {}}').appendedCells).toEqual([]);
    expect(parseOverlay('{"appendedCells": 5}').appendedCells).toEqual([]);
    expect(parseOverlay('{"appendedCells": null}').appendedCells).toEqual([]);
  });
});

describe('parseOverlay — numeric key adversarial cases', () => {
  it('non-integer string keys are dropped', () => {
    const r = parseOverlay('{"cellEdits": {"1.5": "a", "abc": "b", "x2": "c"}}');
    expect(r.cellEdits).toEqual({});
  });

  it('negative indices are KEPT (current behavior; merge never matches them)', () => {
    const r = parseOverlay('{"cellEdits": {"-3": "neg"}}');
    expect(r.cellEdits).toEqual({ '-3': 'neg' });
  });

  it('huge indices are kept', () => {
    const r = parseOverlay('{"cellEdits": {"999999": "big"}}');
    expect(r.cellEdits[999999]).toBe('big');
  });

  it('exponential-notation keys are coerced to integers (quirk: Number("1e3")===1000)', () => {
    const r = parseOverlay('{"cellEdits": {"1e3": "thousand"}}');
...
describe('parseOverlay — appendedCells validation', () => {
  ...
  it('drops appended cells with wrong-typed required fields', () => {
    ...
    expect(parseOverlay(json).appendedCells).toEqual([]);
  });
  ...
  it('accepts afterIndex null', () => {
    ...
    expect(parseOverlay(json).appendedCells).toHaveLength(1);
  });
  ...
  it('keeps extra/unknown fields on appended cells (no whitelist)', () => {
    ...
    const a = parseOverlay(json).appendedCells[0] as AppendedCell & { bogus?: string };
  ...
  it('preserves outputs/executionCount on appended cells WITHOUT validating their shape', () => {
    ...
    const cells = parseOverlay(json).appendedCells;
...
describe('parseOverlay — unicode & large payloads', () => {
  ...
  it('preserves unicode through a raw Uint8Array round-trip', () => {
    ...
    const bytes = new TextEncoder().encode(serializeOverlay(r));
    expect(parseOverlay(bytes).appendedCells[0].source).toBe(src);
  });
  ...
  it('handles a very large base64-ish output payload', () => {
    ...
    const parsed = parseOverlay(serializeOverlay(r));
...
describe('parseOverlay — round-trip stability (deep equality)', () => {
  it('parse(serialize(x)) deep-equals x for a rich recording incl. every output kind', () => {
    ...
    expect(parseOverlay(serializeOverlay(rich))).toEqual(rich);
  });
  ...
  it('is idempotent across two serialize/parse cycles', () => {
    ...
    const once = parseOverlay(serializeOverlay(r));
    const twice = parseOverlay(serializeOverlay(once));
...
describe('isOverlayEmpty', () => {
  it('true for a fresh emptyOverlay', () => {
    expect(isOverlayEmpty(emptyOverlay())).toBe(true);
  });

  it('false when ANY single sub-collection is non-empty', () => {
    const e1 = emptyOverlay(); e1.cellEdits[0] = 'x';
    const e2 = emptyOverlay(); e2.cellOutputs[0] = [];
    const e3 = emptyOverlay(); e3.cellCounts[0] = null;
    const e4 = emptyOverlay(); e4.appendedCells.push({ id: 'a', afterIndex: null, cellType: 'code', source: '' });
    expect(isOverlayEmpty(e1)).toBe(false);
    expect(isOverlayEmpty(e2)).toBe(false); // even an empty outputs array counts as non-empty
    expect(isOverlayEmpty(e3)).toBe(false); // even a null count counts as non-empty
    expect(isOverlayEmpty(e4)).toBe(false);
  });
...
describe('mergeNotebook — output precedence', () => {
  ...
  it('no recorded output → baked-in output shown, outputRecorded=false', () => {
    const m = mergeNotebook(realNotebook(), emptyOverlay())[1];
    if (m.origin === 'ipynb') {
      expect(m.outputRecorded).toBe(false);
...
describe('mergeNotebook — every MergedCell has coherent origin/flags', () => {
  it('pristine notebook + empty recording: all ipynb, all flags false/baked', () => {
    const merged = mergeNotebook(realNotebook(), emptyOverlay());
...
describe('mergeNotebook — degenerate / empty notebooks', () => {
  it('null notebook + empty recording → []', () => {
    expect(mergeNotebook(null, emptyOverlay())).toEqual([]);
  });
...
describe('mergeNotebook — recording references beyond notebook length', () => {
  it('cellEdits/cellOutputs/cellCounts for out-of-range indices are simply never applied', () => {
    ...
    r.cellCounts[50] = 5;
    const baseline = mergeNotebook(realNotebook(), emptyOverlay());
    const merged = mergeNotebook(realNotebook(), r);
  ...
  it('a negative-index edit is parsed but never matched by merge', () => {
    const r = parseOverlay('{"cellEdits": {"-1": "neg"}}');
    const merged = mergeNotebook(realNotebook(), r);
...
// =====================================================================
// integration-ish: parsed recording → merge (no hand-built Overlay)
// =====================================================================

describe('parseOverlay → mergeNotebook end to end', () => {
  it('a recording loaded from bytes merges identically to its in-memory twin', () => {
    ...
    const bytes = new TextEncoder().encode(serializeOverlay(r));
    const fromBytes = mergeNotebook(realNotebook(), parseOverlay(bytes));
